    ...
    while attempt < MAX_ATTEMPTS:
        ...
        def unassign_course_length2(course: Dict, day_idx: int, period_idx: int):
            ...
            initial_solution[day_str] = [
                ...
                if not (a["Subject"] == course["name"] and set(a["Instructors"]) == set(course["instructors"]) and a["periods"]["period"] == period_idx)
            ]

            # コースを未スケジュールとしてマーク
            scheduled_courses.remove(get_unique_course_key(course))
        ...
        def assign_graduation_courses(graduation_courses: List[Dict]) -> bool:
            ...
            for course in graduation_courses:
                assigned = False
                # 「卒業研究」コースは length=2 または length=4 と仮定
                        ...
                        for grade in grade_groups:
                            ...
                            if get_num_periods(course.get("length", 2)) == 1:
                                if can_assign_length2(course, day_idx, period_idx, grade):
                                    ...
                                    assigned = True
                                    break  # 割り当て成功で次のコースへ
                            elif get_num_periods(course.get("length", 2)) == 2:
        ...
        def fill_length4_courses_for_cat3(courses: List[Dict]):
            ...
            # 割り当て可能なスロットが少ない順、かつ教員数が多い順、かつ教室数が多い順にソート
            # これにより、スロットが少なく教員数と教室数が多い授業が優先的に割り当てられます
            cat3_courses_sorted = sorted(
                cat3_courses,
                key=lambda course: (get_num_possible_slots(course), -len(course.get("instructors", [])), -len(course.get("rooms", [])))
            )
        ...
        # 未割り当てのコースがなければスケジューリング完了
        if not unscheduled:
            logging.debug(f"全てのコースを割り当てました。試行回数: {attempt}")
            return initial_solution

        # 未割り当てのコースが存在する場合、リセットして再試行
        logging.debug(f"未割り当てのコースが存在します。試行回数: {attempt}")
        logging.debug(f"未割り当てのコース: {unscheduled}")

    # 最大試行回数に達しても未割り当てのコースが存在する場合
